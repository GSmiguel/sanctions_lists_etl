"""UN Security Council Consolidated List source: full XML -> BigQuery."""
...
def run(
    *,
    ...
    rows into BigQuery (see :mod:`...common.sinks.bigquery`).
    """
    log.info("[un] starting")
    content, provenance = resolve_input(
        SPEC,
        fetch=lambda: download_un_consolidated(url=url),
        local_path=xml_path,
    )
    build = build_rows(
        SPEC,
    ...
    log.info("[un] done: %d records", build.record_count)
